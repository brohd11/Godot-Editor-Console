...
func _execute(ctx:Context):
	...
	for scope_name in scope_names:
		...
		var lines := []
		_render(_instance(scope), 0, lines)
	...
	for scope_name in hidden_names:
		...
		_render(_instance(scope), 0, builtin_lines)
	md += "\n## Builtins\n\n```text\n%s\n```\n" % "\n".join(builtin_lines)
...
func _instance(scope:Dictionary):
	var command = scope.get("script")
	return command.new() if command is GDScript else command
