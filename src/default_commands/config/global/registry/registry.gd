...
const _HELP = \
"Manage classes suggested as script targets.
Usage: config global registry {--add|--rm} <class_name>"
...
func _get_completions(_ctx:Completion):
	var options = Options.new()
	if add:
		var global_class_list = Utils.global_class_paths().keys()
...
func _execute(ctx:Context):
	...
	for nm in positional_args:
		if add:
			...
			if not Utils.global_class_paths().has(nm):
...
func _target_config():
	return Config.get_target_config(not global_flag)
